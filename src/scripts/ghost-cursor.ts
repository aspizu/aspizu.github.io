export interface GhostCursorOptions {
    element?: Element
    randomDelay?: boolean
    minDelay?: number
    maxDelay?: number
    lifeSpan?: number
    image?: string
    zIndex?: number | string
}

type Vec2 = {x: number; y: number}

class Particle {
    readonly initialLifeSpan: number
    lifeSpan: number
    position: Vec2
    image: CanvasImageSource

    constructor(x: number, y: number, image: CanvasImageSource, lifeSpan: number) {
        this.initialLifeSpan = lifeSpan
        this.lifeSpan = lifeSpan
        this.position = {x, y}
        this.image = image
    }

    update(ctx: CanvasRenderingContext2D): void {
        this.lifeSpan--
        const opacity = Math.max(this.lifeSpan / this.initialLifeSpan, 0)
        ctx.globalAlpha = opacity
        ctx.drawImage(this.image, this.position.x, this.position.y)
    }

    get dead(): boolean {
        return this.lifeSpan < 0
    }
}

export class GhostCursor {
    private element: Element
    private hasWrapper: boolean

    private canvas!: HTMLCanvasElement
    private ctx!: CanvasRenderingContext2D
    private raf = 0

    private width = window.innerWidth
    private height = window.innerHeight
    private cursor: Vec2 = {x: this.width / 2, y: this.height / 2}

    private particles: Particle[] = []
    private image = new Image()

    private randomDelay: boolean
    private minDelay: number
    private maxDelay: number
    private lifeSpan: number

    private lastAdd = Date.now()
    private interval = 0

    constructor(private options: GhostCursorOptions = {}) {
        this.hasWrapper = Boolean(options.element)
        this.element = options.element ?? document.body

        this.randomDelay = Boolean(options.randomDelay)
        this.minDelay = options.minDelay ?? 5
        this.maxDelay = options.maxDelay ?? 50
        this.lifeSpan = options.lifeSpan ?? 20

        this.image.src =
            options.image ??
            "data:image/png;base64,"

        this.interval = this.nextDelay()
        this.init()
    }

    private init(): void {
        this.canvas = document.createElement("canvas")
        const ctx = this.canvas.getContext("2d")
        if (!ctx) throw new Error("2D context unavailable")
        this.ctx = ctx

        Object.assign(this.canvas.style, {
            top: "0px",
            left: "0px",
            pointerEvents: "none",
            zIndex: String(this.options.zIndex ?? 9999999999),
            position: this.hasWrapper ? "absolute" : "fixed",
        })

        if (this.hasWrapper) {
            this.element.appendChild(this.canvas)
            this.resizeToElement()
        } else {
            document.body.appendChild(this.canvas)
            this.resizeToWindow()
        }

        this.bind()
        this.loop()
    }

    destroy(): void {
        cancelAnimationFrame(this.raf)
        this.canvas.remove()
        this.unbind()
    }

    private bind(): void {
        this.element.addEventListener("mousemove", this.onMouseMove)
        this.element.addEventListener("touchmove", this.onTouchMove, {passive: true})
        this.element.addEventListener("touchstart", this.onTouchMove, {passive: true})
        window.addEventListener("resize", this.onResize)
    }

    private unbind(): void {
        this.element.removeEventListener("mousemove", this.onMouseMove)
        this.element.removeEventListener("touchmove", this.onTouchMove)
        this.element.removeEventListener("touchstart", this.onTouchMove)
        window.removeEventListener("resize", this.onResize)
    }

    private onResize = (): void => {
        this.hasWrapper ? this.resizeToElement() : this.resizeToWindow()
    }

    private resizeToWindow(): void {
        this.width = window.innerWidth
        this.height = window.innerHeight
        this.canvas.width = this.width
        this.canvas.height = this.height
    }

    private resizeToElement(): void {
        const el = this.element as HTMLElement
        this.canvas.width = el.clientWidth
        this.canvas.height = el.clientHeight
    }

    private onTouchMove = (e: TouchEvent): void => {
        for (const t of Array.from(e.touches)) {
            this.addParticle(t.clientX, t.clientY)
        }
    }

    private onMouseMove = (e: MouseEvent): void => {
        if (this.randomDelay) {
            if (this.lastAdd + this.interval > Date.now()) return
            this.lastAdd = Date.now()
            this.interval = this.nextDelay()
        }

        if (this.hasWrapper) {
            const r = this.element.getBoundingClientRect()
            this.cursor.x = e.clientX - r.left
            this.cursor.y = e.clientY - r.top
        } else {
            this.cursor.x = e.clientX
            this.cursor.y = e.clientY
        }

        this.addParticle(this.cursor.x, this.cursor.y)
    }

    private nextDelay(): number {
        return (
            Math.floor(Math.random() * (this.maxDelay - this.minDelay + 1)) +
            this.minDelay
        )
    }

    private addParticle(x: number, y: number): void {
        this.particles.push(new Particle(x, y, this.image, this.lifeSpan))
    }

    private update(): void {
        if (!this.particles.length) return

        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height)

        for (const p of this.particles) p.update(this.ctx)
        this.particles = this.particles.filter((p) => !p.dead)

        if (!this.particles.length) {
            this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height)
        }
    }

    private loop = (): void => {
        this.update()
        this.raf = requestAnimationFrame(this.loop)
    }
}
